...
async function assertRuntimeReadme(readme: string): Promise<void> {
  ...
  assert.deepEqual(JSON.parse(blocks[0]), expected.opencode)
  assert.deepEqual(JSON.parse(blocks[1]), {"$schema":"https://opencode.ai/config.json","plugin":[`github:CyberRookie-X/auto-resume#v${pkg.version}`]})
